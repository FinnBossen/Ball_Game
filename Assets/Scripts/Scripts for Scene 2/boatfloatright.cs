...
public class boatfloatright : MonoBehaviour {
    ...
    {
        // bewegt Boot nach Rechts wenn Spieler in Trigger
        if (other.tag == "Player")
        {
            if (!boatPath.wall)
            {
              
                Parent.transform.Translate(Vector3.right * moveSpeed * Time.deltaTime);
            }
        }
    }
